...
**依存:** ステップ5（definition.tsのparametersに合わせる）
...
## ステップ7: Execute関数を実装する (plugin.ts)
...
**依存:**
- ステップ5: `TOOL_NAME`を使う
- ステップ6: `GreetingData`, `GreetingArgs`の型を使う
...
**依存:**
- ステップ5: `TOOL_NAME`を使って結果をフィルタリング
- ステップ6: `GreetingData`型でデータを受け取る
- ステップ7: `execute()`が返した`data`を受け取る
...
## ステップ10: Previewコンポーネントを作成する (Vue)

**このステップの目的:** サイドバーに表示する小さなサムネイルを作成する

**影響:** サイドバーの結果一覧に表示される

**依存:** ステップ6（GreetingData型を使用）
...
ステップ3で説明したMock Modeを使ってテストします：

1. `demo/shared/chat-utils.ts`にgreetingのモックレスポンスを追加（ステップ3参照）
2. デモ画面で「Mock Mode」がONになっていることを確認
3. チャットに「挨拶を作って」と入力
4. グリーティングカードが表示される！

Real API Modeでもテストしたい場合は、ステップ3の「Real API Modeに切り替え」を参照してください。
